...
- **Social Security (shape 1).** A derived income `CashFlowSeries`. Requires the simulator to
  **accumulate a lifetime earnings record** as it runs (every income segment contributes), then
  compute the benefit from it. Real mechanics to *approximate* (and disclaim — the true formula
  is complex and legislation-bound): benefit based on highest ~35 years of indexed earnings
  (AIME → PIA bend-point formula); **claiming age is a decision variable** (62 earliest, ~67
  full, delayed to 70 ≈ +8%/yr) — separable from but interacting with the retirement solver (§7);
  COLA-adjusted annually (fits the nominal engine). Subsystem = earnings-record accumulator +
  benefit formula + claiming-age choice (default ~67).
