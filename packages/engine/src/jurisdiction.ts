 ...
 *
 * For now this exposes only the `computeTax` seam; it widens over time
...
/**
 * The canonical government-benefit seam input: the frozen covered-earnings
 ...
 * the worker keeps earning, which the base formula reads to index the
 * record forward.
 */
 ...
 * The engine holds the frozen base benefit as an OPAQUE number and, each
...
/**
 * Context for the retirement-deferral-limit seam. The engine supplies the
...
/**
 * Context for the Required Minimum Distribution seam. The engine owns the
...
/**
 * Context for the health-cost benchmark seam. The engine supplies the
...
export interface Jurisdiction {
  ...
  /**
   * seam (readable fact): the age at which public health coverage begins,
  ...
  /**
   * The single tax chokepoint (seam 1): per-{@link TaxCategory} taxable
  ...
  /**
   * seam (disclosure): the SAME tax total as {@link computeTaxCents},
  ...
  /**
   * seam (disclosure): model simplifications SPECIFIC to this jurisdiction's
   ...
   * lives HERE in `rules`, never in the neutral engine. Optional: absent (v1
   * null jurisdiction) → the report carries only the engine's assumptions.
   */
  readonly modelAssumptions?: readonly ModelAssumption[];

  /**
   * seam: how much of a post-tax account withdrawal is TAXABLE, given the cost
  ...
  /**
   * seam: how an account's periodic RETURN is taxed, given the neutral kind of
   ...
   * categorization out of the engine and into `rules`, where it belongs.
   */
  returnTaxTreatment?(returnKind: AccountReturnKind, ctx: JurisdictionContext): ReturnTaxTreatment;

  /**
   * seam: which income {@link TaxCategory} flows count toward the covered-
  ...
  /**
   * seam: a person's annual employee pre-tax deferral limit (401k-style) for
   ...
   * in the priority order. Optional and legislation-set; when absent,
  ...
  /**
   * seam (readable fact): the default benefit claiming age — the full
  ...
  /**
   * seam (base): the government retirement benefit in eligibility-age dollars —
   ...
   * claim — then again only while the record keeps growing. Optional and
  ...
  /**
   * seam (COLA): apply the cost-of-living adjustment to a frozen base benefit —
   ...
   * is not earned wages. Optional: absent → the base is paid unadjusted.
   */
  colaAdjustedBenefitCents?(baseCents: Cents, ctx: GovernmentBenefitContext): Cents;

  /**
   * seam: the Required Minimum Distribution a pre-tax account holder must
  ...
  /**
   * seam: the attributed monthly health cost for a person of the given age
   ...
   * post-65 gap VISIBLE: the app pre-fills the attributed stepped segment
 ...
 * end to end on its own. Never remove it — it is
