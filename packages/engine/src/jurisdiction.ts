...
export interface GovernmentBenefitClaim {
  ...
  /**
   * {@link claimingAge} at first claim, higher when the base is recomputed while the
   * worker keeps earning.
   */
...
/** Passed once a year per account holder. */
...
export interface Jurisdiction {
  ...
  /**
   * Age public health coverage begins, at which the attributed health cost steps down from
   * self-funded to residual (US law: 65, Medicare). The early-retiree honesty flag measures
   * the self-funded gap up to it. Absent → no step, no gap.
   */
  readonly publicHealthCoverageAge?: number;

  /**
   * The single tax chokepoint. Categories are passed through whole, never collapsed into
   * one lump: the jurisdiction owns how much of each is taxed and at what rate (benefit
   * inclusion %, capital-gains preference).
   */
  computeTaxCents(
   ...
   * preferential gains rate, benefit provisional-income inclusion.
   *
   ...
   *
   * Required, with no fallback; a jurisdiction charging no tax returns `{}`. Reporting only
   * — the scalar {@link computeTaxCents} stays the marginal-tax probe the withdrawal
   * gross-up loop depends on.
   */
  ...
  /**
   * Simplifications specific to this jurisdiction, listed after the engine's neutral
   * {@link import("./projection/assumptions").MODEL_ASSUMPTIONS}. US-specific caveats —
   * e.g. brackets grown forward at a flat rate rather than the IRS's published yearly
   * figures — live here, never in the engine.
   */
   ...
   * jurisdiction owns the return-of-capital POLICY and its accounting method (pro-rata,
   * specific-lot, average-cost); the engine reduces basis by `grossCents − taxable`, which
   * keeps the state update method-agnostic.
   *
   * Called for many probe amounts inside the withdrawal gross-up loop, so it MUST be pure
   * and monotone non-decreasing in `grossCents` — that is what lets the loop climb to its
   * least fixed point. Absent → the whole `grossCents` is taxable.
   */
   ...
   * (capital appreciation). Absent, or `taxAtAccrual: false` → nothing is booked at accrual.
   */
   ...
   * caps combined deferral at this and redirects overflow to the next destination; the
   ...
   * person's benefit is not timed at all.
   */
  readonly defaultBenefitClaimingAge?: number;

  /**
   * The government retirement benefit in eligibility-age dollars — `PIA(record) ×
   * claimingFactor(claimingAge)`. The eligibility gate lives INSIDE here, not on a separate
   * seam: a record that fails it (US: < 40 credits) returns 0. The engine caches the result
   * as an OPAQUE base, recalling this only while the record keeps growing. Absent → 0.
   */
  ...
  /**
   * The jurisdiction owns the eligibility age (US: 65) and the two figures it steps
   * between: before it, an unsubsidised self-funded cost (~$1,200/mo/person, conservative);
   * at/after it, a residual (~$500/mo/person — Medicare replaces self-funded insurance but
   * premiums, Part B, and out-of-pocket remain).
   *
   * NOT a silent auto-step in the sim: the app pre-fills an authored, disclaimed budget
   * item from it, and {@link
   * import("./earlyRetireeHealthCheck").assessEarlyRetireeHealthCost} compares an authored
   * health expense against the pre-eligibility figure. Absent → no benchmark (0).
   */
 ...
 * own, and the baseline for engine-only tests.
 */
