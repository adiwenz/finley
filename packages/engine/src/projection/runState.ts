...
export interface SimState {
  ...
  /**
   * Per-account cost basis — the post-tax principal booked into each account.
  ...
  /**
   * Credited return awaiting accrual-taxation, keyed by ACCOUNT id. For
   ...
   * tax seam. Keyed per account (not per owner) so two cash accounts held by one person
   ...
   * pre-now summary. Every month's covered wages are folded in; the record
   ...
   * cached base benefit. The base is recomputed only when a newer
...
export function initSimState(input: HouseholdSimInput): SimState {
  ...
  for (const acc of input.accounts) {
    ...
    // Opening basis: a pre-tax account has zero basis by definition (its
  ...
  // Synthetic 22% card absorbs shortfalls when no real cards are entered.
  ...
  // the terminal HARD-INFEASIBILITY flag (`isInsolvent`) rather than read
  // as solvent forever.
