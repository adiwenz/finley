...
describe("Federal income tax — the year's balance settles the following April", () => {
  ...
  /**
   * The one thing the year's estimate still approximates: a one-time transfer into or out of an
   * account. The forecast pass PERFORMS it, like any other month would — but the cheap estimate
   * that seeds that pass ({@link import("./taxYearProjection").projectKnownTaxYear}) walks balances
   * forward through draws and growth alone and never sees it. So the pass simulates the right year
   * while charging instalments sized for a different one, and what the two disagree about is what
   * April is left holding.
   *
   * That is now the only lever these tests have, and deliberately so. Everything else they used to
   * pull is anticipated outright: a lumpy spend, a debt's origination and maturity, contributions,
   * basis, an account running dry. A settlement exists for what the seed cannot scale correctly,
  ...
  it("makes the year's instalments plus its balance equal the annual tax on its actual income", () => {
    const rate = 0.25;
    // $24k of wages against $72k of spending, with $60k in checking that the SEED expects to cover
    // the gap — so it prices no pre-tax sale at all. Checking is then emptied in July by a transfer
    // the seed never described, and the forecast pass, which does perform it, funds the year's
    // second half by selling pre-tax under instalments sized for a year with no sales in it.
    const projection = simulateHousehold(
      baseInput(
        [
    ...
    // instalments are far more than the $6,000 the wages alone would explain, because the forecast
    // pass emptied checking in July and did the pre-tax selling that forced.
  ...
  it("funds an April balance from the retirement account as income of the year of the DRAW", () => {
    ...
    // because 2026 collected all but the slice its seed mis-sized — what April sells for, and so
    // what 2027 is taxed on, is that slice rather than the whole year's tax.
...
describe("Federal income tax — a retired household decumulating, over five full years", () => {
  ...
  it("charges every month of every year a flat instalment — December included", () => {
    ...
    for (let y = 0; y < YEARS; y++) {
      ...
      // April is the ONLY month carrying anything extra, and only year `y-1`'s balance — CENTS of
      // it. The estimate is a simulation of the year, so what it leaves behind is the rounding
      // between one twelfth and the next, not a bill.
      expect(Math.abs(months[3]! - months[2]!), `year ${y} April`).toBeLessThan(dollarsToCents(1));
    }
  });
