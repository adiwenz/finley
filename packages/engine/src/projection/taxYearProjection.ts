/**
 * T₀ — the CHEAP estimate, and the seed the year's real estimate is grown from.
 *
 * It projects the annual taxable income knowable at the start of a tax year without running the
 * stateful funding waterfall, and prices it. What it produces is never the year's schedule: it is
 * what the forecast pass CHARGES while it simulates the year, and that pass's own December total
 * is the estimate (`priceTaxYear` in {@link import("./simulate").simulateHousehold}).
 *
 * Which is why this can afford to be rough about timing and must not be rough about SCALE. A
 * seeded month's tax is itself a cash need, funded by taxable withdrawals; seed too low and the
 * forecast pass draws too little, under-reports the income those draws realize, and the year
 * under-collects by the whole circular slice. Detail the forecast pass gets right for free — a
 * March event, an account depleting in August, contributions, basis, a one-time transfer — this
 * only has to get approximately right, because the simulation corrects it.
 *
 ...
 *
 * That fixed point is over THIS function's own forecast, not over the simulation: it is a handful
 * of arithmetic passes, and the pass that consumes it runs once.
 *
 ...
 * import("./taxYearSettlement").finalizeTaxYear}). A better seed does not make the year's tax more
 * or less correct — it makes more of it arrive evenly through the year the income was earned in,
 * leaving a smaller balance to carry into the next.
 */
