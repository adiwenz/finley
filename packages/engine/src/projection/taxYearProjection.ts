 ...
 *
 * The simulator is a simulator: at the start of a tax year it already holds every scheduled
 * flow that year will bring — the compiled income series (wages, pensions, alimony), the
 * government benefit, the year's RMD. So the estimate reads those directly. It is emphatically
 * NOT `taxableIncomeYTD / monthsElapsed * 12`: annualizing year-to-date income charges a lumpy
 * earner a large estimate in the month a lump lands and refunds it back over the months after,
 * which is the sawtooth the annual model exists to remove.
 *
 * Deliberately EXCLUDED: endogenous taxable withdrawals (a decumulation draw, a home
 * purchase's funding sale). They arise from the funding waterfall months later and cannot be
 * known here; the tax they cause is settled in December's reconciliation instead — which is
 * also why the estimate is only an estimate, and why {@link
 * import("./runState").SimState.taxableIncomeByPersonYear} stays the authoritative base.
 */
