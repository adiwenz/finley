...
describe("Desired-withdrawal decumulation channel", () => {
  ...
  it("draws exactly the need from a pre-tax account, ignoring a flat mid-year tax entirely", () => {
    // A jurisdiction that WOULD tax the ordinary-income draw at 25% has no effect on the
    // draw's size: federal income tax is never charged against an ordinary mid-year draw —
    // only the December settlement (annualTaxSettlement.ts) ever grosses up.
