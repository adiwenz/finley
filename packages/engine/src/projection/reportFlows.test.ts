...
// The engine's answer to "what is this person earning from work right now", so no consumer has
// to add up bands and re-invent the blend. Every case below is stated in terms of what the sim
// BANDED: a job that paid nothing this month books no source, and so cannot be counted.
describe("buildFlows — wagesByOwner", () => {
  it("rolls each earner's paying jobs up separately, counting the jobs that paid", () => {
    const flows = buildFlows(
      [
        src("p1", 5_000_00, "wages", { sourceId: "job:a", label: "Job A" }),
        src("p1", 2_000_00, "wages", { sourceId: "job:b", label: "Job B" }),
        src("p2", 3_000_00, "wages", { sourceId: "job:c", label: "Job C" }),
      ],
      0,
      [],
    );
    expect(flows.wagesByOwner).toEqual({
      p1: { jobCount: 2, grossCents: 7_000_00, deferralCents: 0, deferralFraction: 0 },
      p2: { jobCount: 1, grossCents: 3_000_00, deferralCents: 0, deferralFraction: 0 },
    });
  });

  it("blends the deferral across jobs, weighted by each one's gross pay", () => {
    // $1,000 deferred out of $10,000 across two jobs electing 10% and 0% on unequal pay —
    // 10%, not the 5% an unweighted average of the two elections would give.
    const flows = buildFlows(
      [
        src("p1", 8_000_00, "wages", { sourceId: "job:a", label: "Job A" }),
        src("p1", 2_000_00, "wages", { sourceId: "job:b", label: "Job B" }),
      ],
      0,
      [],
    ...
    expect(flows.wagesByOwner.p1).toEqual({
      jobCount: 2,
      grossCents: 10_000_00,
      deferralCents: 1_000_00,
      deferralFraction: 0.1,
    });
  });

  it("counts pay for work only — benefits, interest and the savings drawdown are not wages", () => {
    const flows = buildFlows(
      [
        src("p1", 4_000_00, "wages", { sourceId: "job:a", label: "Job A" }),
        src("p1", 2_000_00, "governmentRetirementBenefit", { sourceId: "benefit:p1", label: "Benefit" }),
        src("p1", 0, "ordinaryIncome", { cashInflowCents: 40_00, sourceId: "interest:p1", label: "Interest" }),
      ],
      0,
      [],
      1_000_00, // living partly off savings
    );
    expect(flows.wagesByOwner).toEqual({
      p1: { jobCount: 1, grossCents: 4_000_00, deferralCents: 0, deferralFraction: 0 },
    });
  });

  it("gives an earner with no wages this month no entry at all, rather than a zero one", () => {
    // The absence is the signal a consumer reads through `EMPTY_MONTHLY_WAGES` — a retiree
    // drawing a benefit is not "earning $0 across 0 jobs" by way of some banded source.
    const flows = buildFlows(
      [src("p1", 2_000_00, "governmentRetirementBenefit", { sourceId: "benefit:p1", label: "Benefit" })],
      0,
      [],
