...
describe("buildFlows", () => {
  ...
  it("surfaces the Social Security slice as its own convenience field", () => {
    const flows = buildFlows([src("p1", 2_500_00, "socialSecurity"), src("p1", 4_000_00, "wages")], 0, 0);
    expect(flows.socialSecurityCents).toBe(2_500_00);
  });

  it("reports 0 Social Security when no source carries that category", () => {
    const flows = buildFlows([src("p1", 4_000_00, "wages")], 0, 0);
    expect(flows.socialSecurityCents).toBe(0);
  });
