 ...
 * the run's own — so a caller holding working balances rather than the run's can price a month by
 * the same rule the real month is priced by.
 *
 ...
 * and the payment actually funded, in that order. PURE, so a caller can walk a debt forward on
 * its own working balances through the exact rule the run advances by, rather than a lookalike
 * that drifts from it.
 */
