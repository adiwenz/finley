/**
 * The standing authoring account model (§9, §10 of JOBS_HOUSEHOLD_REDESIGN,
 * issue #68, slice 5) — per-person account *ownership* and the household net
 * worth aggregate that #30 (multi-person panel) and #38 (account types) build on.
 *
 * This is the *authoring* account, deliberately distinct from the lower-level
 * simulator {@link import("./account").Account} class (a compiled shape with a
 * single `ownerId`, rate segments and one-time transfers that the month-by-month
 * sim consumes). The standing account carries only what the user authors:
 * identity, the set of `owners`, a balance, and whether it is a retirement
 * vehicle. Keeping the two as separate types is the same seam that keeps standing
 * `Person` out of the sim's `SimPerson` (see `person.ts`).
 *
...
/**
 * A standing, user-authored account (§10). `owners` distinguishes individual
...
/**
 * A standing household (§8, §9): the persons and the single canonical account
 ...
 */
export interface StandingHousehold {
  readonly persons: readonly Person[];
  readonly accounts: readonly StandingAccount[];
}
 ...
 */
export function assertAccountOwnership(account: StandingAccount): StandingAccount {
...
/**
 * Author a {@link StandingAccount}, enforcing the ownership invariants at the
 * point of creation (see {@link assertAccountOwnership}).
 */
export function makeStandingAccount(params: {
  id: string;
...
export function isJoint(account: StandingAccount): boolean {
  return account.owners.length > 1;
}

/** An individual account — held by exactly one person. */
export function isIndividual(account: StandingAccount): boolean {
  return account.owners.length === 1;
}
 ...
 * include this person.
 */
export function jointAccounts(household: StandingHousehold, person: PersonRef): StandingAccount[] {
  const id = idOf(person);
 ...
 */
export function accountsOf(household: StandingHousehold, person: PersonRef): StandingAccount[] {
  const id = idOf(person);
 ...
 */
export function householdNetWorthCents(household: StandingHousehold): Cents {
