 ...
 * `./projectionRun`, the retirement search in `../retirement/retirementOutlook` — so which plane a job is
...
import type { Plan, PlanPatch, GoalPatch } from "../plan/plan";
import type { JobIncomeOverrideInput, JobPayChangeInput, PersonId } from "../job/job";
import type { BudgetLineOverride, BudgetLinePatch } from "../budget/budgetLine";
...
export class Projection {
   ...
   * {@link import("../job/householdJob").continuationJobIdOf}). A surface offering the choice has to
   ...
   * none — see {@link import("../plan/person").Person.continuationJobId}.
   *
  ...
  /**
   * A dated, source-directed cash outflow — never refused on affordability; a shortfall
   * against the named sources blocks the PROJECTION at `input.month` instead, and the event
   * stays authored. Returns the minted `"spend-N"` id.
   */
