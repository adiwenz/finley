...
import { eventEntryType } from "./scenarioInput";
...
describe("ScenarioInput shape", () => {
  it("declares a whole scenario with refs and no ids", () => {
    const input: ScenarioInput = {
      ...
      jobs: [
        {
          ...
          deferral: { deferralFraction: 0.1, fundAccountRef: "retirement" },
        },
      ],
      goals: [
      ...
      budgetLines: [
        { ref: "rent", label: "Rent", category: "needs",
          amountSource: { kind: "literal", monthlyCents: 200_000 },
          target: { kind: "expense" } },
        { ref: "iraContrib", label: "IRA", category: "savings",
          amountSource: { kind: "fillToLimit" },
          target: { kind: "account", accountRef: "retirement", taxTreatment: "preTax" } },
      ],
      events: [
        { type: "takeLoan", ref: "student", month: 0, ownerRef: "primary",
          openingBalanceCents: 3_000_000, apr: 0.05, kind: "studentLoan", termMonths: 120 },
        { type: "payOffDebt", month: 60, liabilityRef: "student", accountRef: "retirement",
          amountCents: 500_000 },
      ],
    };
  ...
  it("maps each event variant to its own discriminant", () => {
    const entries: readonly EventEntry[] = [
      ...
      { type: "takeLoan", month: 0, ownerRef: "primary", openingBalanceCents: 100_000,
        apr: 0.2, kind: "creditCard", creditLimitCents: 500_000 },
      { type: "buyHome", month: 36, ownerRef: "primary", purchasePriceCents: 40_000_000,
        downPaymentCents: 8_000_000, downPaymentSourceRefs: ["savings"], mortgageApr: 0.06,
        mortgageTermMonths: 360 },
      { type: "separate", month: 48, partnerRef: "sam" },
      { type: "payOffDebt", month: 60, liabilityRef: "card", accountRef: "savings",
        amountCents: 100_000 },
    ];
