/**
 * The engine's export gateway — ONE module stating an application's entire dependency on
 * `@finley/engine`, so adding to it is a deliberate edit with a reason beside it.
 *
 * It declares nothing. {@link Projection} is the whole authoring surface and the whole read
 * surface over authored state; it lives in `./projectionFacade`, and the state transformations it
 * delegates to live in `./authoring/*`. Everything else in the package — the simulator, the
 * ledger, the waterfall, the snapshot/report pipeline, the entity transforms — is internal and
 * reached by relative path from inside this package only.
 *
 * The line, drawn once: if answering the question needs the projection — the plan, the ledger,
 * the household, the run — it is a METHOD on `Projection`, and it returns what the caller
 * actually wanted rather than a compiler artifact to finish assembling. What a caller may name
 * beyond that is here: the types its methods take and return, a well-known id to quote, a unit
 * conversion with no state to convert against, a label for an enum value.
 *
 ...
 *
 * Named re-exports only, never `export *`: the app's `planWrites.guard.test.ts` reads this file's
 * text to decide what the app may name, so a wildcard would widen the surface silently.
 */

// The facade itself, and the artifacts its two derived-output queries answer with.
export { Projection } from "./projectionFacade";
export type { ProjectionResult, RetirementOutlook } from "./projectionFacade";

// The authoring state a caller holds, and the version contract that decides whether a
// persisted one can be reopened.
export type { ProjectionState } from "./authoring/state";
export { CURRENT_FORMAT_VERSION, UnsupportedVersionError } from "./authoring/restore";
