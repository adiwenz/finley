 ...
 * waterfall and accumulates in its account, rather than being dropped or — worse — modelled
 * as an expense that leaves net worth. These pin the end-to-end fact through the real
 * projection: a "$500/mo into brokerage" line moves money account→account (net worth is not
 * reduced) and the brokerage balance climbs.
 *
 * A contribution is also a COMMITTED outflow: the full amount always lands, and the part
 * discretionary can't cover is borrowed (a shortfall), so an unaffordable auto-invest makes
 * the plan unfinanceable rather than silently shrinking to fit.
 */
...
describe("budget contribution lines fund their account", () => {
  ...
  it("does NOT reduce net worth — it moves cash account→account, unlike an expense", () => {
    ...
    // Idling $500/mo in cash (savings rate) vs investing it (brokerage rate) — routing to
    // the higher-returning account never lowers net worth. An "expense" model would instead
    // make netB ~$12k LOWER.
  ...
  it("is committed: the FULL contribution lands even when discretionary can't cover it", () => {
    // Take-home ~$7,200/mo ($8k gross − 10% deferral). Rent $6,500 + $600 health leaves
  ...
  it("makes the plan unfinanceable when the contribution is far beyond your means", () => {
    // $1,000,000/mo into brokerage on an ~$7.2k take-home can't be borrowed for long: the
    // shortfall cascade exhausts savings and credit within the first year.
    const absurd: Plan = {
      ...samplePlan,
      goals: [],
  ...
  it("does NOT inflate net worth when a committed contribution overshoots into insolvency", () => {
    // A $10,000,000/mo contribution you can't fund must not show as a ~$10M net-worth spike
    // in the insolvent month. The part that couldn't be funded (from cash, savings, or
    // credit) is unwound, so net worth stays near the pre-contribution level rather than
    // booking a phantom asset.
    const absurd: Plan = {
      ...samplePlan,
      goals: [],
  ...
  it("an overshoot covered by savings is a neutral transfer — no phantom, and NOT insolvent", () => {
    // The boundary that makes the phantom an INSOLVENCY-only effect: a $5,000/mo
    // contribution overshoots the ~$3,600 discretionary, but $20k opening savings absorbs
    // the ~$1,400/mo gap for over a year — the overshoot is funded for REAL (savings drawn
    // down), not borrowed into thin air. Net worth stays ~unchanged vs no contribution (a
    // neutral cash→brokerage transfer), never inflated by the $5,000, and the month stays
    // solvent. Below insolvency there is nothing to unwind.
