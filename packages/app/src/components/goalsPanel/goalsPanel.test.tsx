...
describe("GoalsPanel", () => {
  ...
  it("shows a Funded badge once a goal's projected fund reaches target on/before the date", () => {
    ...
    const html = renderToStaticMarkup(
      <GoalsPanel budget={budget} series={project(budget)} transact={noWrites} ledger={emptyLedger} />,
    );
  ...
  it("invites a first goal when the plan has none", () => {
    ...
    const html = renderToStaticMarkup(
      <GoalsPanel budget={empty} series={project(empty)} transact={noWrites} ledger={emptyLedger} />,
    );
