...
export interface UseLedger {
  ...
  /**
   * Revise timeline events, keeping each id and its place in the log. A revision that would
   * strand a later event is rejected as a conflict, like a blocked removal.
   *
   * **All or nothing**: revisions replay onto one ledger value and commit only if every one
   * is accepted. The `false` return keeps a caller spanning both authoring planes atomic —
   * the Jobs panel revises an event *and* edits the plan, and must not write the plan side
   * if the ledger refused.
   */
   ...
   * matching base via `buildPresetLedger`); this clears stale conflicts.
   */
...
export function useLedger(base: LedgerBaseConfig): UseLedger {
  ...
  // The latest ledger, readable *synchronously*: `reviseEvents` must report acceptance before
  // its caller writes the other plane, which a functional updater (result a render later)
  // cannot do. Mirrored every render and on each accepted revision, so two revisions in one
  // tick still see each other.
