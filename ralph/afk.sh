...
if [ -z "$1" ]; then
  ...
  echo "  commit           checks out a per-issue branch (ralph/issue-<n>),"
  echo "                   commits changes there, and closes/comments the issue"
...
# In commit mode, isolate each issue's work on its own branch (ralph/issue-<n>),
# creating it the first time and reusing it after. No-op in local (report-only)
# mode, and a no-op when we're already on that branch — e.g. when afk-issue.sh
# drives us inside an isolated clone it has already put on the branch. Runs on
# the host; the sandbox mounts this working tree, so the agent's commits land on
# whichever branch we switch to here.
ensure_branch() {
  local issue="$1" branch="ralph/issue-$issue"
  [ "$MODE" = "commit" ] || return 0
  [ "$(git rev-parse --abbrev-ref HEAD 2>/dev/null)" = "$branch" ] && return 0
