...
# build/push commands in the PR/README). Vercel sandboxes run on linux/amd64, so
...
# expose it on the system PATH and make it readable so a non-root sandbox user
# (Vercel may run as one) can execute it too.
