...
supplied at construction (defaulting to `nullJurisdiction`), independent of the jurisdiction
`run()` takes per call. Authoring asks "is this write permitted?"; `run()` asks "what does this
...
- **GREEN** — Threaded a construction-time `validationJurisdiction` through `Projection`;
  ...
  three companion tests (null accepts, default is null, `run()` stays independent) stay green.
- **Full suite** — 1076 passed / 45 todo across all packages; engine purity and typecheck clean.
...
- **Default `nullJurisdiction`.** Keeps the engine runnable standalone with no rules package, so
  no existing caller of `create`/`fromJSON` changes behaviour.
...
- `packages/engine/src/projectionRoot.ts`
  ...
  - `static create(init, jurisdiction = nullJurisdiction)` and
    `static fromJSON(state, jurisdiction = nullJurisdiction)` accept and thread it.
...
- `packages/engine/src/projectionRoot.test.ts`
  - New describe block "authoring validates against the construction-time jurisdiction" with a
    local `flatCapitalGains` mock jurisdiction and four tests covering refusal, null-accept,
    the default, and `run()` independence.

## Verification & Testing

- `npm run check:purity` → engine purity passed (no rules import leaked into the engine).
- `npm run typecheck` → clean.
- `npx vitest run packages/engine` → **607 passed | 45 todo**.
- `npm run test` (whole workspace) → **1076 passed | 45 todo**, 85 files.
